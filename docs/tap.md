...
- Linux 6.6 or newer, for TCX. On an older kernel the program cannot attach: the daemon logs `tap program on <interface>: TCX needs Linux 6.6 or newer` (once per interface), `shukractl programs` shows `tap` as `detached`, and `shukractl doctor` reports the kernel (`kernel`, info). There is no TC fallback yet.
...
|---|---|---|
| `tap  detached`, the log says `TCX needs Linux 6.6 or newer` (doctor `kernel`, info) | The kernel has no TCX. There is no TC fallback | Upgrade to 6.6 or newer. The programs that follow the VMM (`kvm`, `sched`, `block`, `net`, `vmm`) are unaffected |
