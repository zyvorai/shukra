...
- Connect, flow and inbound events together are capped at 200 per tap per second, and DNS names have a separate budget of the same size, so a guest that floods SYNs, invents flows or resolves names in a loop cannot flood a ring or starve another kind of event. The counters still see every packet.
