...
`deploy/install.sh` is the one install routine shared by the source deploy (`scripts/deploy-remote.sh`), the tarball and the `.deb`; `scripts/test-install.sh` tests it. The notes in [v0.1.0](releases/v0.1.0.md) match the `v0.1.0` tag. A later tag needs its own note brought up to date the same way. [the CHANGELOG](../CHANGELOG.md) lists what has merged to `main`, newest first.
