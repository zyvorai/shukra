...
- **DNS beyond the query name.** Names of queries over UDP port 53 are recorded (`guest_dns`). Answers are not read, so a name is not mapped to the addresses it resolved to. Queries over TCP port 53 and DNS over TLS or HTTPS are not read, nor are second questions or compressed names; a guest that reaches a resolver over TLS is still seen going there by the server name in its hello (`guest_tls`), unless Encrypted Client Hello hides it.
