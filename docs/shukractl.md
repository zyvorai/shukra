...
| `policy [<vm>]`, `policy learn <vm>`, `policy apply <vm> --mode audit\|enforce\|off [--allow a,b \| --from-baseline] [--confirm 10m \| --permanent]`, `policy confirm <vm>`, `policy remove <vm>` | Which networks a VM may start connections to. Bare, it lists each VM's policy with what it has counted; `learn` proposes one from the baseline; `apply` puts a VM under one (audit first: it drops nothing; enforcing needs `--confirm` or `--permanent`); `confirm` keeps an enforcing policy; `remove` lifts one. Changes need the admin key. See [egress policy](egress-policy.md) |
