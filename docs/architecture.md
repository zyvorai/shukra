...
- **A new link is not waited on.** `RTM_NEWLINK` and `RTM_DELLINK` run the same tap sync as the two-second scan, then the saved egress policy, before the tap is reported as covered. The refresh is debounced by 50 ms across a burst. A missed notification is caught by the scan. An enforcing VM with a live tap and no program raises `tap-uncovered`. `-quarantine-uncovered` is off: it would drop that tap, except the management allow list, until the policy is written. The same socket records link, address, route and neighbor changes as events (`host-netlink`). A VM tap is joined to that VM; `guest_attributed` stays false. A deleted or down tap, a moved master, a removed tap address or changed MTU, a deleted default route, a failed neighbor and a lost Netlink message are detections, and they keep `host-netlink`. `-netlink-events=false` stops the events and those detections and leaves the refresh. See [Netlink](netlink.md).
...
- **Events are 2048 in all, but not one queue.** A busy host produces a few kinds by the hundreds a second (a k3s node's connects, slow-block samples), and a plain oldest-first queue lets them push out the rare ones an operator is looking for: a guest's DNS name, a connection made into a VM, a detection. Each kind belongs to a class with a share no other class can take:
  ...
  | host network | `tcp_connect`, `tcp_retransmit`, `netlink_link`, `netlink_address`, `netlink_route`, `netlink_neighbor`, `netlink_error` | 512 |
