...
| `-netlink-events` | on | Record host link, address, route and neighbor changes, and the detections made from them. `-netlink-events=false` stops those events and detections; a link change still refreshes tap discovery. See [Netlink](../netlink.md) |
