...
`shukractl vms` lists `qemu-system-*` processes. `runtime=libvirt` or `runtime=kubevirt` is a label from the command line, not a guest agent. `taps=` is what Shukra found for the VM, from `ifname=` on the command line or, for libvirt, from the process's tun file descriptors. A VM with `taps=-` is on user-mode networking or its tap could not be read, and `shukractl doctor` says which, so its guest traffic is not seen and it cannot be isolated.
