...
| `-netlink-events=false` | Do not record host link, address, route and neighbor changes, or the detections made from them. A link change still refreshes tap discovery. See [Netlink](../netlink.md) |
...
`shukractl programs` prints eight lines: `kvm`, `sched`, `block`, `mem`, `net`, `vmm` and `drops` `attached` with a hook count, and `tap` `attached` with how many VM taps it is on (or `detached: no VM tap interfaces to attach to yet` on a host with no VMs) when BTF and clang were available, and `shukractl status` says `programs    8/8 attached`. On a host with no clang or BTF (or with `SHUKRA_ALLOW_DETACHED=1` after a failed build) they are `detached`, and the deploy log said `programs will report detached` or `CO-RE objects were not generated`. Detached is a successful install of the control plane.
