...
> **Before you turn this on.** A management allow list enables *isolate* and *enforce*, which means anyone holding the admin key can now cut a VM off, and the API the key opens may be reachable from further than you think. If `shukractl doctor` reports `The API key is the well-known dev key` or `The API is plain HTTP on a non-loopback address` (the deploy script's defaults on a lab host: key `shukra`, port `30970` on `0.0.0.0`, no TLS), fix that first: a real key in `/etc/shukra/env`, TLS or `-listen 127.0.0.1:30970` ([deploy](03-deploy.md#encrypt-the-api)). The dev key is public knowledge and plain HTTP shows the key to the network. Do not add an allow list to a host in that state.
