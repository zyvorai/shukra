...
| `GET /api/v1/policy[?vm=]` | The [egress policies](egress-policy.md): per VM the `mode` (`audit` or `enforce`), `allow` networks, `source`, who set it and when, `present`, any `revert` (an enforcing policy waiting to be confirmed: `until`, `to`), any `problem`, and per tap the `kernel` mode and `checked`, `auditPackets`, `auditBytes`, `droppedPackets` and `droppedBytes`. Also `orphans` (taps the kernel applies a policy on that nobody recorded), `persisted`, and `enabled` |
| `GET /api/v1/policy/proposal?vm=` | What the VM's learned baseline proposes as its list (`allow`), how that differs from its current policy (`current`, `added`, `removed`), and whether it is still learning. `409` with baselines off |
| `POST /api/v1/policy/apply` | Admin key. `{"vm", "mode": "audit"\|"enforce"\|"off", "allow"?: [...], "fromBaseline"?: true, "confirm"?: "10m", "permanent"?: true}`. Enforcing needs `confirm` or `permanent`, the management allow list and `-data-dir`. `400` a request that is wrong, `404` no such VM, `409` refused, each with the reason as text |
| `POST /api/v1/policy/confirm`, `POST /api/v1/policy/remove` | Admin key. `{"vm"}`. Confirm makes an enforcing policy stay; remove lifts one, and puts a tap right that has one nobody recorded |
| `GET /api/v1/actions?all=1` | What responses decided. `enabled` says whether the rules file has any, `pending` how many wait for a person, and `actions` lists the pending ones (or, with `all=1`, everything held, newest first): `id`, `vm`, `response`, `mode`, `dryRun`, the detection's `rule`, `severity` and `message`, `created`, `expires`, `status` (`pending`, `executed`, `released`, `refused`, `rejected`, `expired`, `dry_run`), `decidedBy`, `result`, `guardrail` and `releaseAt`. See [responses](responses.md) |
| `POST /api/v1/actions/<id>/approve`, `POST /api/v1/actions/<id>/reject` | Admin key only. Approving isolates the VM (the guardrails are checked again). `404` for an unknown id; `409`, with the action and the reason, when it is not waiting, has expired, or a guardrail or the isolate itself refused it; `409` when no responses are configured. The `X-Shukra-Actor` header is recorded as who decided |
| `GET /api/v1/actions/<id>/incident` | The incident bundle captured when the action was made, as `GET /api/v1/incident` would have returned it then |
