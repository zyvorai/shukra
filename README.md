...
| What is this VM connecting to, who connects to it, and what names does it look up or ask for? | `guest_connect`, `guest_flow`, `guest_inbound`, `guest_dns` and `guest_tls` events, seen on the VM's own tap. `guest_tls` is the server name and fingerprint in a TLS ClientHello, so a site is named even when DNS was not used |
...
- **No agent, no payloads.** Shukra never runs inside a guest and never reads application data. The three things it reads beyond headers are the name in a plain DNS query over UDP port 53 (`-dns-events=false` turns that off), the first segment of a TLS ClientHello, for its server name (`-tls-events=false` makes the program read no TCP payload at all), and the **names of the files a QEMU process opens** (never their contents; `-vmm-tripwires=false` turns that off); a ClientHello is sent before anything is encrypted and holds no application data. Answers, the rest of a TLS connection, and everything else are not read. It can say *which VM and which address*, not *which process inside the guest*. The guest's own CPU steal counter is not read; the host's view of the same thing, vCPU preemption and who caused it, is measured.
