...
func TestGuestTrafficRequiresTCX(t *testing.T) {
	st := healthy(t)
	st.kernelRelease = func() string { return "5.15.0-generic" }
	c := byID(st.Doctor(), "kernel")
	if c == nil || c.Status != "info" || strings.Contains(c.Detail, "clsact") || !strings.Contains(c.Detail, "unavailable") {
		t.Fatalf("%+v", c)
	}
	st.SetPrograms([]Program{{Name: "tap", Status: "detached", Detail: "TCX needs Linux 6.6 or newer"}})
	c = byID(st.Doctor(), "program-tap")
	if c == nil || strings.Contains(c.Title+c.Detail, "clsact") || !strings.Contains(c.Detail, "TCX needs Linux 6.6") {
		t.Fatalf("%+v", c)
	}
}
