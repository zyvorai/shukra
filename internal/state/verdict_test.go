...
func TestNothingToSayWhenNothingIsMeasured(t *testing.T) {
	if got := causes(diagnose(false, nil, nil, nil, nil, nil)); len(got) != 1 || got[0] != "unknown_vm:high" {
		t.Fatalf("%v", got)
	}
	got := diagnose(true, []aggregate.KVMRow{{}}, []aggregate.SchedRow{{}}, nil, []aggregate.BlockRow{{}}, nil)
...
func TestHostCPUContentionNeedsAVCPUThreadToBeHigh(t *testing.T) {
	...
	f := find(diagnose(true, quiet, sched, threads, nil, nil), "host_cpu_contention")
	...
	f = find(diagnose(true, quiet, sched, nil, nil, nil), "host_cpu_contention")
	...
	if f = find(diagnose(true, quiet, sched, mid, nil, nil), "host_cpu_contention"); f == nil || f.Confidence != "medium" {
		t.Fatalf("%+v", f)
	}
	low := []aggregate.ThreadRow{{TID: 11, Role: "vcpu", WakeupDelayP99Ns: 300_000}}
	if f = find(diagnose(true, quiet, []aggregate.SchedRow{{Measured: true}}, low, nil, nil), "host_cpu_contention"); f != nil {
...
func TestStorageLatencyThresholds(t *testing.T) {
	...
	for _, c := range []struct {
		...
		f := find(diagnose(true, quiet, nil, nil, blk(c.read, c.write), nil), "storage_latency")
		if c.want == "" {
			if f != nil {
	...
	if f := find(diagnose(true, quiet, nil, nil, blk(1*ms, 60*ms), nil), "storage_latency"); !strings.Contains(f.Evidence[0], "write") {
...
func TestKVMExitHandlingSkipsHaltsWhenNamingTheCostliestExit(t *testing.T) {
	...
	f := find(diagnose(true, k, nil, nil, nil, nil), "kvm_exit_handling")
	...
	if f = find(diagnose(true, k, nil, nil, nil, nil), "kvm_exit_handling"); !strings.Contains(f.Evidence[1], "reason 123") {
		t.Fatalf("%+v", f)
	}
	k[0].LatencyP99Ns = 2 * ms
	if f = find(diagnose(true, k, nil, nil, nil, nil), "kvm_exit_handling"); f == nil || f.Confidence != "medium" {
		t.Fatalf("%+v", f)
	}
	k[0].LatencyP99Ns = 200_000
	if find(diagnose(true, k, nil, nil, nil, nil), "kvm_exit_handling") != nil {
...
func TestRetransmitsAreLowConfidenceAndSaySoAboutWhoseTraffic(t *testing.T) {
	...
	f := find(diagnose(true, quiet, nil, nil, nil, net), "tcp_retransmits")
	if f == nil || f.Confidence != "low" || !strings.Contains(f.Summary, "not the guest") {
		t.Fatalf("%+v", f)
	}
	if find(diagnose(true, quiet, nil, nil, nil, []aggregate.NetRow{{Retransmits: 3}}), "tcp_retransmits") != nil {
...
func TestQuietHostSaysThePauseMayBeInsideTheGuest(t *testing.T) {
	got := diagnose(true, quiet, []aggregate.SchedRow{{Measured: true, WakeupDelayP99Ns: 4096}}, nil,
		[]aggregate.BlockRow{{Measured: true, ReadP99Ns: 1 * ms}}, nil)
...
func TestFindingsAreRankedBySupport(t *testing.T) {
	...
	got := causes(diagnose(true, quiet, sched, threads, block, net))
