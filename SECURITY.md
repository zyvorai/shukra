...
- **A guest that can reach the daemon's API address can try its key.** The shipped unit listens on `0.0.0.0:30970`, so a guest whose network reaches the host can reach it, and the bearer key is the only barrier. Bind to a management address (`-listen`) or firewall it (see [the API](#the-api)).
...
- The API is plain HTTP unless you pass `-tls-cert` and `-tls-key`, and the daemon warns when it serves plain HTTP on a non-loopback address. `SIGHUP` reloads a renewed certificate and keeps the old one if the new one does not load.
...
- **The shipped unit listens on `0.0.0.0:30970` over plain HTTP.** With plain HTTP the bearer key crosses the network in the clear. Use `-tls-cert` and `-tls-key`, or bind to `127.0.0.1` or a management address, or firewall the port.
